...
function EditMenuItem() {
    ...
    const fetchMenuItem = async () => {
        try {
            ...
            setFormData(data);
            setSelectedSizes(data.sizes.map(size => ({ label: size.name, value: size._id })));
            setSelectedIngredients(data.extraIngredientPrices.map(ingredient => ({ label: ingredient.name, value: ingredient._id })));
        } catch (error) {
